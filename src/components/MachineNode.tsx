...
export function MachineNode({ data, selected }: NodeProps<AppNode>) {
  ...
  return (
    <div className={`px-4 py-3 shadow-xl rounded-lg bg-slate-900 border-2 text-white min-w-[180px] transition-colors ${selected ? 'ring-2 ring-blue-500 ring-offset-2 ring-offset-slate-950' : ''} ${statusStyles[data.status]}`}>
      ...
      <div className="space-y-1.5 text-[11px] text-slate-400">
        <div className="flex justify-between">
          <span className="flex items-center gap-1"><Timer size={12}/> Speed</span>
